...
var d = (r, e, s) => e in r ? u(r, e, { enumerable: !0, configurable: !0, writable: !0, value: s }) : r[e] = s;
...
class p {
  ...
  addElementToDom() {
    ...
    e ? (this.element = document.createElement("canvas"), e.appendChild(this.element), this.resize()) : console.error("canvas not created");
  }
  getDimensions() {
  ...
  resize() {
    this.getDimensions(), this.element.width = this.adjustedWidth(), this.element.height = this.adjustedHeight();
  }
  getContext() {
    if (this.context)
      return this.context;
...
class l {
  ...
  async getAdapter() {
    return this.adapter ? this.adapter : (this.gpuCapable || this.raiseNotCapable(), this.adapter = await navigator.gpu.requestAdapter(), this.adapter);
  }
  async getDevice() {
    if (this.device)
      return this.device;
    this.gpuCapable || this.raiseNotCapable();
    const e = await this.getAdapter(), s = e && await e.requestDevice();
...
class f {
  ...
  getFormat() {
    return this.format ? this.format : (this.format = navigator.gpu.getPreferredCanvasFormat(), this.format);
  }
  getCurrentTexture() {
    return this.context.getCurrentTexture();
  }
  getDepthTextureFormat(e = m) {
    if (this.depthTextureFormat)
      return this.depthTextureFormat;
    const { height: s, width: i } = this.canvas.getAdjustedDimensions();
    return this.depthTextureFormat = this.device.createTexture({
      size: [i, s],
      format: e,
...
class b {
  constructor(e, s) {
    ...
    t(this, "buffers");
    t(this, "shaders");
    this.gpuApp = e, this.depthTesting = !1, this.depthTestingOptions = T, this.entryOptions = C, this.buffers = [], this.shaders = s;
  }
  build() {
  ...
  setDepthTesting(e = {}) {
    return this.depthTesting = !0, this.depthTestingOptions = {
      ...this.depthTestingOptions,
      ...e
    }, this;
  }
  setEntry(e) {
    return e.vertex && (this.entryOptions.vertex = e.vertex), e.fragment && (this.entryOptions.fragment = e.fragment), this;
  }
  buildVertex(e) {
    return {
      module: e,
...
class E {
  constructor(e, s, i) {
    ...
    t(this, "pipeline");
    t(this, "passEncoder");
    this.renderer = s, this.gpuApp = e, this.device = e.device, this.shaders = i, this.vertexCount = 0, this.models = [];
  }
  overrideVertexCount(e) {
    this.vertexCount = e;
  }
  run() {
    this.getPassEncoder(), this.buildPipeline(), this.addPipeline(), this.setupBindGroups(), this.draw();
  }
  getPassEncoder() {
...
class P {
  constructor(e = D) {
    ...
    t(this, "deltaTime");
    t(this, "onUpdate");
    this.startTime = performance.now(), this.rate = 0, this.deltaTime = 1, this.onUpdate = e;
  }
  update(e = performance.now()) {
    this.calculate(e), this.startTime = e, this.onUpdate(this);
  }
  calculate(e) {
    this.deltaTime = e - this.startTime, this.rate = Math.round(3600 / this.deltaTime);
  }
}
class y {
  constructor(e) {
...
class O {
  ...
  render(e = n) {
    this.update(e), this.createFrameResources(), this.renderPipelines.forEach((s) => {
      s.run();
    }), this.finishFrame();
  }
  addPipeline(e) {
  ...
  setupEncoder() {
    const s = A(this.gpuApp).background(
      this.backgroundColor || F
    );
    return s.view = this.gpuApp.context.getCurrentTexture().createView(), this.commandEncoder.beginRenderPass({
      colorAttachments: [s]
    });
  }
}
...
class R {
  ...
  async setupDevice() {
    return this.device ? this.device : (this.device = await g(this.context), this.renderer.setup(), this.device);
  }
  getTextureInfo() {
    return this.textureInfo ? this.textureInfo : (this.textureInfo = v(this.canvas, this.device), this.textureInfo);
  }
  getFormat() {
  ...
  setupCanvas(e = o) {
    return this.canvas ? this.canvas : (this.canvas = c(e), this.context = this.canvas.context, this.canvas);
  }
  ...
  resetCanvas(e) {
    this.canvas.reset(), this.context = this.canvas.context, this.configureCanvas(e);
  }
  onCanvasResize() {
...
const B = async (r = {}) => {
  const e = new R();
  return e.setupCanvas(r.parentSelector || o), await e.setupDevice(), e.configureCanvas(), window.addEventListener("resize", () => e.onCanvasResize()), e;
}, h = (r) => {
  const { a: e } = r;
  return {
    r: r.r * e,
...
}, S = (r, e) => e === "opaque" ? r : h(r), k = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  normalizeColor: S,
  premultiply: h
}, Symbol.toStringTag, { value: "Module" })), I = `fn premultiply(color: vec4f) -> vec4f {
